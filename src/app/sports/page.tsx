...
export default async function SportsPage() {
  const [articles] = await Promise.all([
    api.getArticles({ category: 'sports' }),
  ]);
  ...
  return (
    <div className="bg-brand-surface min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
        ...
        {/* Header */}
        <div className="bg-white rounded-brand p-6 border-l-4 border-amber-500 shadow-sm mb-6">
          ...
          <h1 className="font-heading font-black text-2xl sm:text-4xl text-brand-dark">
            Unity Sports Arena & FUFA Drum
          </h1>
          <p className="text-xs sm:text-sm text-gray-600 mt-1 max-w-2xl">
            Live match commentary, FUFA Drum Lango Province updates, Akii-Bua Olympic Stadium coverage, Uganda Premier League, and school championships.
          </p>
        </div>
            ...
            >
              <div>
                ...
                >
                  ...
                  />
                  <span className="absolute bottom-2 left-2 bg-amber-500 text-black text-[10px] px-1.5 py-0.5 rounded font-black">
                    {story.location_tag}
                  </span>
                </Link>
                <div className="p-4">
                  <Link href={`/news/${story.category.slug}/${story.slug}`}>
                    <h3 className="font-heading font-black text-base text-brand-dark group-hover:text-amber-600 transition-colors line-clamp-2 mb-2">
                      {story.title}
                    </h3>
                  </Link>
