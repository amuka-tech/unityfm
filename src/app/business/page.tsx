...
import { Wheat, TrendingUp, ArrowRight } from 'lucide-react';

export default async function BusinessPage() {
  const [articles] = await Promise.all([
    api.getArticles({ category: 'business' }),
  ]);
  ...
  return (
    <div className="bg-brand-surface min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
        ...
        {/* Header */}
        <div className="bg-white rounded-brand p-6 border-l-4 border-emerald-600 shadow-sm mb-6">
          <div className="flex items-center space-x-2 text-xs font-bold text-emerald-700 uppercase tracking-wider mb-1">
            <Link href="/" className="hover:underline">Home</Link>
            <span>/</span>
            <span>Business & Agriculture</span>
          </div>
          <h1 className="font-heading font-black text-2xl sm:text-4xl text-brand-dark">
            Agribusiness & Commercial Markets
          </h1>
          <p className="text-xs sm:text-sm text-gray-600 mt-1 max-w-2xl">
            Tracking Northern Uganda’s dynamic agricultural value chains — Nilotica Shea Butter, soya bean processing, commercial grain SACCOs, forex, and regional market commodity prices.
          </p>
        </div>
            ...
            >
              <div>
                ...
                >
                  ...
                  />
                  <span className="absolute bottom-2 left-2 bg-emerald-800 text-white text-[10px] px-1.5 py-0.5 rounded font-bold">
                    {story.location_tag}
                  </span>
                </Link>
                <div className="p-4">
                  <Link href={`/news/${story.category.slug}/${story.slug}`}>
                    <h3 className="font-heading font-black text-base text-brand-dark group-hover:text-emerald-700 transition-colors line-clamp-2 mb-2">
                      {story.title}
                    </h3>
                  </Link>
